...
namespace cmstar.Caching
{
    ...
    public abstract class CacheProviderTestBase
    {
        ...
        /// 当前测试用例中使用的缓存超时。
        /// 耗时较短的版本，主要用于验证超时。
        /// 由于<see cref="CacheExpiration"/>的精度为妙，此超时至少需要1秒钟。
        /// </summary>
        public static readonly TimeSpan ExpiryShort = TimeSpan.FromMilliseconds(1000);

        /// <summary>
        /// 当前测试用例中使用的缓存超时。
        /// 用于测试缓存值，避免因网络延迟等原因，还没来得及获取到值就过期了。
        /// </summary>
        public static readonly TimeSpan ExpiryLong = TimeSpan.FromMinutes(2);
        ...
        public void TestExpirationControl()
        {
            ...
            // 等缓存超时。
            Thread.Sleep(ExpiryShort.Add(TimeSpan.FromMilliseconds(1200)));

            // 验证缓存已经被清理了。
            Assert.IsNull(CacheProvider.Get<string>(Key));

            // 缓存不存在的情况下，Remove 应当返回 false。
