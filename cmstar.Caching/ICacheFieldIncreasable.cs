...
namespace cmstar.Caching
{
    ...
    public interface ICacheFieldIncreasable : ICacheFieldAccessable
    {
        ...
        /// <returns>true若对已存在的缓存对象的对应字段进行了设值；否则返回false。</returns>
        bool FieldIncrease<T, TField>(string key, string field, TField increment, out TField result);
        ...
        /// </returns>
        /// <remarks>'Cx' means 'Create if not eXists'.</remarks>
        bool FieldIncreaseCx<T, TField>(string key, string field, TField increment, TimeSpan expiration, out TField result);
    }
}